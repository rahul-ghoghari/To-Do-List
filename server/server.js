...
app.get("/getData/:email", function (req, res) {
    ...
    console.log(email);
    List.aggregate([{ $match: { email: email } }, { $sort: { dueDate: 1 } }]).then((list) => {
        if (list == null) {
...
app.put("/UpdateList/:id", function (req, res) {
    ...
    List.findByIdAndUpdate({ _id: id }, { title: req.body.title, dueDate: new Date(req.body.dueDate) })
        .then((list) => {
            console.log(list);
            if (list == null)
...
app.post("/addList", function (req, res) {
    ...
    var insert = List.collection.insertOne({ title: req.body.title, email: req.body.email, dueDate: new Date(req.body.formDate) });
    if (insert) {
